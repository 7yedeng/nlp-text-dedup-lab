...
# 署名不写死：默认用“成员一+成员二”，与 build_report.py 的环境变量口径一致
_m = [os.environ.get(k, "").strip() for k in
      ("REPORT_MEMBER1_NAME", "REPORT_MEMBER1_ID",
       "REPORT_MEMBER2_NAME", "REPORT_MEMBER2_ID")]
NAME = "+".join(_m) if all(_m) else "成员一+成员二"
...
def main():
    ...
    md.append(f"> 附件：中文文本特征表示与内容重复理解实验（第 1 次实验）　成员：{NAME}\n")
