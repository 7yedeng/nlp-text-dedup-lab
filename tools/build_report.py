...
# ---------------------------------------------------------------- 成员署名
# 姓名与学号**不写死在代码里**：默认取环境变量，未设置时用占位符。
# 这样公开仓库里的生成脚本本身不含个人信息；
# 需要出学校提交版时，在本地设置环境变量即可（不要写进仓库）：
#   $env:REPORT_MEMBER1_NAME="<姓名>"; $env:REPORT_MEMBER1_ID="<学号>"
#   $env:REPORT_MEMBER2_NAME="<姓名>"; $env:REPORT_MEMBER2_ID="<学号>"
#   python tools/build_report.py
_PH = "__________________"
MEMBERS = [
    ("成员一", os.environ.get("REPORT_MEMBER1_NAME", "").strip() or _PH,
     os.environ.get("REPORT_MEMBER1_ID", "").strip() or _PH),
...
# 分工表里的成员显示名：未填时用“成员一/成员二”，避免把占位符连成一串
M1_LABEL = MEMBERS[0][1] if MEMBERS[0][1] != _PH else "成员一"
M2_LABEL = MEMBERS[1][1] if MEMBERS[1][1] != _PH else "成员二"
...
def build():
    ...
    for line in ["实验名称：文本特征表示与短文本去重 / 文本相似度计算",
                 "实验课程：自然语言处理",
                 f"{MEMBERS[0][0]}：{MEMBERS[0][1]}　　　学号：{MEMBERS[0][2]}",
                 f"{MEMBERS[1][0]}：{MEMBERS[1][1]}　　　学号：{MEMBERS[1][2]}",
                 f"班级：{CLASS_NAME}",
                 f"实验日期：{EXP_DATE}"]:
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    # ===== 成员信息与分工（不编号说明页）=====
    h(doc, "实验小组成员", 1)
    add_table(doc, ["成员", "姓名", "学号"],
              [[m[0], m[1], m[2]] for m in MEMBERS],
              cap="表 0-1 实验小组成员", widths=[1.2, 2.0, 2.4])

    h(doc, "小组分工说明", 1)
    para(doc, "本实验采用**双人合作、AI 代理参与**的完成方式。"
              "下表记录两位成员实际承担的工作（每个实验环节由两人分别负责原理学习、"
              "代码阅读与结果检查，以均摊工作量）；"
              "**AI 生成代码、自动执行程序及生成报告等工作，统一在「AI 工具使用说明」中披露，"
              "不作为成员独立手工编写代码的经历。**"
              "表中留空的项目由成员根据实际参与情况补充——**没有做过的人工检查不要填成已完成。**")
    add_table(doc, ["工作环节", "负责成员", "实际参与内容（可据实补充）"], [
        ["作业要求梳理、任务安排与 AI 工具操作",
         f"{M1_LABEL}、{M2_LABEL}",
         "共同梳理四组实验的指定参数与提交要求；提出任务并核对 AI 产出"],
        ["实验一(1)：TF-IDF 原理学习、代码阅读与结果检查",
         M2_LABEL,
         "阅读 TF-IDF 与分词处理代码；核对 Top-10 是否为 10 个不同文本对；"
         "检查词权重与词频关系"],
        ["实验一(2)：MinHash＋LSH 原理学习、代码阅读与结果检查",
         M1_LABEL,
         "阅读 MinHash 签名与 LSH 分桶代码；核对估计误差与候选对数量"],
        ["实验二(1)：词向量原理学习、代码阅读与结果检查",
         M2_LABEL,
         "阅读词向量加载与平均池化代码；核对词对相似度与聚类指标"],
        ["实验二(2)：SimHash 原理学习、代码阅读与结果检查",
         M1_LABEL,
         "阅读指纹与海明距离代码；核对等权与加权两种方法的 P/R/F1 表"],
        ["数据来源整理、报告审阅与提交材料整理",
         f"{M1_LABEL}、{M2_LABEL}",
         "整理数据来源说明与 20 篇新闻明细；通读报告并核对图表编号与结论表述"],
    ], cap="表 0-2 小组分工记录", widths=[1.95, 1.05, 2.9], size=8.5)

    # ===== AI 工具使用说明（不编号说明页）=====
    h(doc, "AI 工具使用说明", 1)
    h(doc, "使用工具", 3)
    add_table(doc, ["项目", "记录"], [
        ["主要 AI 代理工具", "DeepSeek Harness，简称 dsh（代理运行框架）"],
        ["主要调用模型", "deepseek-v4-flash-vision-exp"],
        ["工具版本", "未记录"],
        ["使用时间", "______年____月____日 至 ______年____月____日"],
        ["补充使用的 AI 工具", "ChatGPT，用于本项目的审计讨论、结果解释检查及报告整理建议"],
    ], cap="表 0-3 AI 工具使用记录", widths=[1.7, 4.0])
    para(doc, "说明：DeepSeek Harness（dsh）是**代理运行框架**，"
              "`deepseek-v4-flash-vision-exp` 是**所调用的模型**，二者不是同一项。"
              "此外需区分两个不同的“模型”概念：本节记录的是**开发时调用的语言模型**；"
              "实验二(1) 加载并评估的是**中文预训练词向量模型**（腾讯 AI Lab 轻量版），"
              "属于实验对象，已单列于 5.1.2 节的模型身份表中，二者不混为一谈。")

    h(doc, "使用范围与参与程度声明", 3)
    for s in [
        "**本实验采用 AI 代理主导的完成方式。**实验代码、数据获取与处理脚本、测试及校验脚本、"
        "可视化代码、报告生成脚本和报告文本，主要由 DeepSeek Harness（dsh）调用 "
        "`deepseek-v4-flash-vision-exp` 模型生成；程序执行、调试和修改也主要通过 AI 代理调用工具完成。"
        "AI 的参与贯穿实验完成流程，并非仅用于文字润色或局部代码补全。",
        "本项目还使用 ChatGPT 对代码实现、评估口径、结果一致性及报告表述进行审查和讨论。"
        "**相关 AI 审查与自动化测试不等同于独立人工复核**，也不能单独保证实验结论正确。",
        "实验中的相似度、运行时间、聚类指标及 Precision、Recall、F1 等计算性结果，"
        "以实验脚本运行产生的输出文件（`out/*_results.txt` 与 `out/results_summary_*.json`）为依据，"
        "**不以语言模型直接生成的数值作为实验测量结果**。报告如实保留实验结果与预期不一致的情况"
        "（例如 5.2.6 节中“TF-IDF 加权并未优于等权”的结论）。",
        "本组不将 AI 生成的代码与文字表述为两位成员独立手工编写的成果。"
        "成员实际完成的任务设置、工具操作、代码阅读、原理学习、结果检查及材料整理等工作，"
        "按实际情况记录在上面的分工表中。AI 生成内容仍可能存在错误，"
        "其使用范围及提交方式应遵循任课教师的要求。",
    ]:
        para(doc, s)
    para(doc, "附注：作业要求“独立完成，不得抄袭，不得使用网络下载的现成完整解决方案”，"
              "但文档未明确 AI 工具的允许参与程度。本组按实际使用方式如实披露，"
              "并将在提交前就此与任课教师确认。")
    doc.add_page_break()
    ...
    # 按作业要求的命名格式：姓名+学号+姓名+学号+第1次实验报告.docx
    # 署名未设置时退回“成员一+成员二”，避免把占位符连成超长文件名
    if HAS_REAL_NAMES:
        prefix = f"{MEMBERS[0][1]}+{MEMBERS[0][2]}+{MEMBERS[1][1]}+{MEMBERS[1][2]}"
    else:
        prefix = "成员一+成员二"
    ...
    try:
        ...
        for m in MEMBERS:
            if m[1] != _PH:
                SUBST.append((m[1], f"（{m[0]}姓名）"))
            if m[2] != _PH:
                SUBST.append((m[2], f"（{m[0]}学号）"))
        replaced = 0
